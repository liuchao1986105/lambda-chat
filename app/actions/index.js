...
export const  socket = (process.env.NODE_ENV === 'production') 
      ? io('http://106.14.30.242:4000',{'force new connection': true})
      : io('http://localhost:4000',{'force new connection': true});
    ...
    return (dispatch,getState) => {
        ...
        if(roomInfo.isPrivate){
            if(!state['activeList'][roomInfo.curRoom]){
                ...
                //console.log("privateUser:"+privateUser.avatar)       
                 //console.log("roomInfo:"+roomInfo.avatar)
                dispatch(addActiveItem({
                    roomName: roomInfo.curRoom,
                    avatar: privateUser ? privateUser.avatar : 'https://assets.lambda-study.com/lambda-user.png',
                    isPrivate: true
                }));
            }
            // 如果之前房间不是私聊则清除公共房间记录
...
export const signUp = (nickname,avatar,email,info)=>{
    return new Promise((resolve,reject) => {
        socket.emit('signUp',{nickname,avatar,email,info},(info)=>{
            resolve(info);
        })
    })
}
