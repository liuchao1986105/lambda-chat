...
const handleEnter = (nextState,replace) => {
    ...
    let search, array, nickname, avatar, email, info;
    if(window.location.search){
        //localStorage.clear();
        search = window.location.search;
        array = search.split('&');
        nickname = decodeURI(array[0].substr(6));
        avatar = array[1].substr(7);
        email = array[2].substr(6);
        info = array[3].substr(5);
        info = decodeURI(info);
        localStorage.setItem('currentUser',nickname);

        signUp(nickname,avatar,email,info).then((resault)=>{
             // if(resault.isError){
             // } else{
                localStorage.setItem('token',resault.jwt);
                const token = resault.jwt;
                handleInit({token,device});
                browserHistory.push('/');
            //}
        });
    }else{
